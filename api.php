...
if (basename(__FILE__) == basename($_SERVER["SCRIPT_FILENAME"])) {
    ...
    if (empty($action)) sendJsonResponse(['error' => 'Action is required.'], 400);
    ...
    try {
        switch ($action) {
            ...
            case 'staff_login':
                ...
                if (empty($phone) || empty($pin)) sendJsonResponse(['success' => false, 'error' => 'Phone and PIN are required.'], 400);
                ...
                if (!empty($response['records'])) {
                    sendJsonResponse(['success' => true, 'staff' => $response['records'][0]]);
                } else {
                    sendJsonResponse(['success' => false, 'error' => 'Invalid credentials or inactive user.'], 401);
                }
                break;
            ...
            case 'get_records':
                ...
                if ($table === 'Bookings' && isset($_GET['date'])) {
                    ...
                    // תיקון: רק הזמנות ללא תדירות חוזרת
                    ...
                    $pageSize = intval(sanitizeInput($_GET['pageSize'] ?? '100'));
                    $pageSize = min(100, max(1, $pageSize));
                    $offset = sanitizeInput($_GET['offset'] ?? null);
            ...
            case 'update_record':
                ...
                // תיקון: אם מעדכנים כרטיסיה, עדכן אוטומטית את Remaining Washes
